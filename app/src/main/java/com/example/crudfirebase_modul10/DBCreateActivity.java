package com.example.crudfirebase_modul10;
...
public class DBCreateActivity extends AppCompatActivity {
    ...
    private void deleteDosen(String key) {
        database.child("dosen").child(key).removeValue().addOnSuccessListener(this, new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Snackbar.make(findViewById(R.id.bt_submit), "Data berhasil dihapus", Snackbar.LENGTH_LONG).show();
            }
        });
    }
    ...
    // Static method untuk menghapus data
    public static void deleteDosenData(Activity activity, String key) {
        DatabaseReference database = FirebaseDatabase.getInstance().getReference();
        database.child("dosen").child(key).removeValue().addOnSuccessListener(activity, new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Snackbar.make(activity.findViewById(android.R.id.content), "Data berhasil dihapus", Snackbar.LENGTH_LONG).show();
            }
        });
    }
}
